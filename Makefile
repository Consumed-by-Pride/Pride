...
#   ./pride --emit-llvm output.ll source.pie        # emit LLVM 22 IR text
...
redteam: all
	...
	@if [ -d redteam ]; then \
		...
		echo "redteam/ not present in this checkout (gitignored) — skipping."; \
	fi
...
# ── Convenience: compile a .pie file all the way to a native binary ───────────
# Usage: make compile SRC=examples/fib.pie OUT=fib OPTLEVEL=3
SRC  ?= examples/fib.pie
OUT  ?= a.out
OPTLEVEL ?= 2
...
compile: $(SRC:.pie=.o) runtime/compiler_rt.o runtime/compiler_rt_arch.o
	$(LLD) \
