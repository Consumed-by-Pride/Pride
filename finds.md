...
- `build.sh` and `Makefile` use the root files. The files in `compiler/` seem to be a ghost version (e.g., `compiler/codegen.c3` is 211KB and references "Pride", whereas `./codegen.c3` is 296KB and references "Pride").
...
- **codegen.c3**: `is_main_fn`/`is_main` name checks (deciding whether to
  ...
  (leftover from the Pride→Pride rename) so it could never match the real
...
- **Build system**: `Makefile`'s `compile`/`runtime` targets and
  ...
  named `pride`) — this alone made the conformance harness silently
...
rather than by compiling `pride`/`pride` itself, while the two C runtime
