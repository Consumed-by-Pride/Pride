...
- **`stdlib/effect_async.pie` + `stdlib/effect_async/`** (8 modules): Untyped Higher-Order & Scoped Effect (`HOSE`) Asynchronous I/O and Structured Concurrency suite (`driver.pie`, `epoll_handler.pie`, `uring_handler.pie`, `nursery.pie`, `timeout.pie`, `bracket.pie`, `fiber_pool.pie`, plus `effect_async.pie`) providing stack-reifying algebraic effect handlers and stack-switching fibers for non-blocking socket I/O (`EpollEffectHandler`, `UringEffectHandler`), structured task nurseries (`Nursery`, `nursery_spawn`, `nursery_wait_all`), delimited timeout scopes (`TimeoutScope`), asynchronous RAII resource bracketing (`AsyncBracket`), and fiber-driven work-stealing I/O pools (`FiberIoPool`) without colored async/await functions.
